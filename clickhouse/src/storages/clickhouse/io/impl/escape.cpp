...
std::string FormatDatetime64(Rep source) {
    ...
    const auto tics = std::chrono::duration_cast<time_resolution>(source.GetUnderlying().time_since_epoch()).count();
    const size_t prefix = tics / ratio;
    const size_t suffix = tics % ratio;
    return fmt::format("toDateTime64('{0}.{1:0>{2}}', {2})", prefix, suffix, Rep::TagType::kPrecision);
}
