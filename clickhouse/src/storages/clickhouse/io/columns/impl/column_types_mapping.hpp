...
auto GetTypedColumn(const clickhouse::impl::clickhouse_cpp::ColumnRef& generic_column_ptr) {
    auto column_ptr = generic_column_ptr->As<NativeColumnType>();
    if (!column_ptr) {
        throw std::runtime_error{fmt::format(
            "failed to cast column of type '{}' to '{}'",
...
auto NativeGetAt(const clickhouse::impl::clickhouse_cpp::ColumnRef& column, size_t ind) {
